# app.py — Funky Brain LIVE (Stable + Experimental + In-app Combiner + Smarter Cleaning + Raw-Clean Button)
...
def clean_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # إن كان الملف الخام يحتوي عمودًا واحدًا بصيغة "ts,segment,multiplier" نفكّه
    if len(df.columns) == 1:
        single = df.columns[0]
        if all(k in single for k in ["ts", "segment", "multiplier"]):
            df = df[single].astype(str).str.split(",", expand=True)
...
with st.sidebar:
    ...
    # 🔁 زر الدمج من مجلد data
    st.markdown("**دمج المجلد:** يجمع كل ملفات `data/spins_cleaned*.csv(xlsx)` إلى `data/combined_spins.csv`.")
    if st.button("🔁 دمج ملفات data/spins_cleaned*.csv(xlsx)"):
    ...
    # 🧹 منظف ملف خام سريع
    st.markdown("---")
    st.markdown("**تنظيف ملف خام وإضافته:** ارفع ملفًا خامًا (من Instant Data Scraper مثلًا)، نُنظّفه ونضيفه للـcombined.")
    raw_up = st.file_uploader("ارفع الملف الخام (CSV/XLSX/XLS)", type=["csv","xlsx","xls"], key="raw_cleaner")
    if st.button("🧹 تنظيف + إضافة إلى combined_spins.csv"):
        if raw_up is None:
            st.error("حمّل ملفًا خامًا أولًا.")
        else:
            try:
                if raw_up.name.lower().endswith(".csv"):
                    raw_df = pd.read_csv(raw_up)
                else:
                    raw_df = pd.read_excel(raw_up)

                before_unknown = int((raw_df.get("segment","UNKNOWN").astype(str).str.upper()=="UNKNOWN").sum()) if "segment" in raw_df.columns else None

                cleaned = clean_df(raw_df)

                os.makedirs(DATA_DIR, exist_ok=True)
                if os.path.exists(REPO_COMBINED_PATH):
                    existed = pd.read_csv(REPO_COMBINED_PATH)
                    merged = (
                        pd.concat([existed, cleaned], ignore_index=True)
                ...
                after_unknown = int((merged["segment"]=="UNKNOWN").sum())
                st.success(f"تم تنظيف الملف وإضافته. إجمالي الصفوف الآن: {len(merged):,}")
                if before_unknown is not None:
                    st.caption(f"معلومة: UNKNOWN قبل التنظيف (إن وُجد عمود segment خام) = {before_unknown}")
